...
use Auth;
use Str;
class Re_VisitorController extends Controller
{
    public function list(){
        $data['getRecord']=User::getVisitor();
    return view('receptionist.visitor.list',$data);   
    }
   
    public function add(){

      
    return view('receptionist.visitor.add');  
    }
    ...
    {
        $data['fees'] = Fee::where("level_id", $request->level_id)
                                ->get(["fees", "id"]);
...
$student->visit_purpose=trim($request->visit_purpose);
...
$student->user_type=7;
$student->save();
return redirect('receptionist/visitor/list')
...
public function edit($id)
    { 
        ...
        if(!empty($data['getRecord']))
        {
            ...
            return view('receptionist.visitor.edit',$data);
        }
        else{
            abort(404);
        }
...
$student->save();
return redirect('receptionist/prospective/list')
->with('success','Student successfully updated');
    ...
    public function delete($id){
        ...
        return redirect()
        ->back()
    ->with('success',"Visitor successfully Deleted");
    }
}
